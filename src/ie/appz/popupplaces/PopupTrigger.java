...
public class PopupTrigger extends Service implements
	...
	public void onCreate() {
		...
		networkListener = new LocationListener() {
			public void onLocationChanged(Location location) {
				Log.i(this.getClass().getName(),
						"New Network Location: " + location.getLatitude() + ","
								+ location.getLongitude() + " - "
								+ location.getAccuracy());
				if (location.getAccuracy() < 40) {
					if (lastLocation == null
							|| (location.getTime() - lastLocation.getTime()) > 4000)
		...
		gpsListener = new LocationListener() {
			public void onLocationChanged(Location location) {
				Log.i(this.getClass().getName(),
						"New GPS Location: " + location.getLatitude() + ","
								+ location.getLongitude() + " - "
								+ location.getAccuracy());
	...
	public void processLocation(Location location) {
		...
		if (shortestDistance < 100) {
			if (placeReached(location.getAccuracy())) {
				...
				Log.i(this.getClass().getName(),
						"Adjusting Network Provider to notify at "
								+ shortestDistance / 2 + "m.");
				mLocationManager.removeUpdates(networkListener);
				mLocationManager.requestLocationUpdates(
						LocationManager.NETWORK_PROVIDER, 30000,
						shortestDistance / 2, networkListener);
				Log.i(this.getClass().getName(),
						"Adjusting GPS Provider to notify at "
								+ shortestDistance / 2 + "m.");
				mLocationManager.removeUpdates(gpsListener);
				mLocationManager.requestLocationUpdates(
						LocationManager.GPS_PROVIDER, 45000,
						shortestDistance / 2, gpsListener);
			}
		} else if (shortestDistance < 150
				&& shortestDiff > oldShortestDistance / 4) {
			this.hasRestarted = false;
			Log.i(this.getClass().getName(),
					"Adjusting Network Provider to notify at "
							+ shortestDistance / 2 + "m.");
			...
			mLocationManager.requestLocationUpdates(
					...
					shortestDistance / 2, networkListener);
			Log.i(this.getClass().getName(),
					"Adjusting GPS Provider to notify at " + shortestDistance
							/ 2 + "m.");
			mLocationManager.removeUpdates(gpsListener);
			mLocationManager.requestLocationUpdates(
					LocationManager.GPS_PROVIDER, 45000, shortestDistance / 2,
					gpsListener);
		} else if (shortestDiff > oldShortestDistance / 4) {
			this.hasRestarted = false;
			Log.i(this.getClass().getName(),
	...
	public int onStartCommand(Intent intent, int flags, int startId) {
		...
		if (!settings.getBoolean(PlaceOpenHelper.SERVICE_DISABLED, false)) {
			Location oldLocation = mLocationManager
...
/*
				...
				if (extras != null
						&& extras.containsKey(ReminderMap_Activity.ItemChanged)
						&& extras
								.containsKey(ReminderMap_Activity.NewLongitude)
						&& extras.containsKey(ReminderMap_Activity.NewLatitude)) {
					{
						boolean change = extras
								.getBoolean(ReminderMap_Activity.ItemChanged);

						if (change) {
							FireableLocation newLocation = new FireableLocation(
									"FromUser");
							newLocation
									.setLatitude(extras
											.getInt(ReminderMap_Activity.NewLatitude) / 1E6);
							newLocation
									.setLongitude(extras
											.getInt(ReminderMap_Activity.NewLongitude) / 1E6);
							Log.d(this.getClass().getName(), "Adding Key: "
									+ oldLocation.distanceTo(newLocation)
									+ " Location: " + newLocation.getLatitude()
							...
							newLocation
									.setLongitude(extras
											.getInt(ReminderMap_Activity.NewLongitude) / 1E6);
							Log.d(this.getClass().getName(), "Removing Key: "
									+ oldLocation.distanceTo(newLocation)
									+ " Location: " + newLocation.getLatitude()
									+ "," + newLocation.getLongitude());
							initializeLocation_UpdateFromDatabase();
			...
			mLocationManager.removeUpdates(gpsListener);
			mLocationManager.requestLocationUpdates(
					LocationManager.GPS_PROVIDER, 60000, 10, gpsListener);
			/*
	...
	private void initializeLocation_UpdateFromDatabase() {
		...
		if (lastLocation != null) {
			...
			Toast.makeText(this, "Centering map on your location.",
	...
	private boolean popupNearest() {
		if (!popupTreeMap.isEmpty()) {
			...
			if (popupText != null && !popupPlace.isFired()
				...
				nCompatBuilder.setContentTitle(this
